...
class ProfitViewSet(viewsets.GenericViewSet):
    ...
    def balance(self, request):
        ...
        total_investment = soup.select('ul.clear em.tfclr')[0].text.replace("원","").replace(',', '')
        residual_investment_price = soup.select_one('div.myinfo_box4 strong.tfclr').text.replace("원","").replace(',', '')

        num_product_url = "https://www.pro-fit.co.kr/mypage/investitems.php"
        res = requests.get(num_product_url, cookies=session.cookies)
        res.raise_for_status()

        soup = BeautifulSoup(res.text, "html.parser")

        number_of_investing_products = soup.select("ul.clear li span.tfclr")[0].text
