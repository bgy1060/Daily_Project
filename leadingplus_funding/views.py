...
class LeadingPlusViewSet(viewsets.GenericViewSet):
    ...
    def account(self, request):
        ...
        try:
            ...
            url_login = "https://www.leadingplusfunding.com/bbs/login_check.php"
        ...
        # 마이페이지에 접근하기
        url_mypage = "https://www.leadingplusfunding.com/mypage/my_info.php"
        res = requests.get(url_mypage, cookies=session.cookies)
        res.raise_for_status()

        if '나의 추천코드' not in res.text:
            username = request.user.register_set.get(company_id=company_id).username.strip()
            password = request.user.register_set.get(company_id=company_id).user_password.strip()
        ...
        account_holder = soup.select("ul.info_com em")[2].text
        bank = soup.select('ul.info_com em')[4].text
        account_number = soup.select('ul.info_com em')[3].text
        deposit = soup.select('ul.info_com em')[1].text.replace("원","")
    ...
    def balance(self, request):
        ...
        # 마이페이지에 접근하기
        url_mypage = "https://www.leadingplusfunding.com/mypage/investitems.php"
        res = requests.get(url_mypage, cookies=session.cookies)
        res.raise_for_status()
        ...
        total_investment = soup.select("ul.clear em.tfclr")[1].text.strip().replace('원', '').replace(',', '')
        residual_investment_price = soup.select_one("li.bordtop em").text.strip().replace('원', '').replace(',', '')
        number_of_investing_products = soup.select("ul.clear em.tfclr")[0].text.strip().replace('건','')
