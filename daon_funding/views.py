...
class DaonViewSet(viewsets.GenericViewSet):
    ...
    def is_valid(self, request):
        ...
        login_info = {
            "webId": USER,  # 아이디 지정
            "member_pwd": PASS  # 비밀번호 지정
        }

        url_login = "https://www.daonfunding.com/loginProcess"
        res = session.post(url_login, data=login_info)
        res.raise_for_status()  # 오류가 발생하면 예외가 발생합니다.

        if '4' in res.text or '3' in res.text :
    ...
    def account(self, request):
        ...
        # 마이페이지에 접근하기
        url_mypage = "https://www.daonfunding.com/member/home"
        ...
        account_holder = soup.select_one("span.p-5-10.bgm-white.c-client-1.brd-50.f-14.f-600").text
        bank = soup.select('td.my-home-tbl-td-border')[3].text.split("\n")[1]
        account_number = soup.select('td.my-home-tbl-td-border')[3].text.split("\n")[2]
        deposit = soup.select('td.my-home-tbl-td-border')[1].text.replace("원","")
    ...
    def balance(self, request):
        ...
        total_investment = soup.select("div.p-t-10.c-client-1.f-12.f-600")[0].text.strip().replace('원', '').replace(',', '')
        residual_investment_price = int(soup.select("div.p-t-10.c-client-1.f-12.f-600")[1].text.strip().replace('원', '').replace(',', '')) \
                                    + int(soup.select("div.p-t-10.c-client-1.f-12.f-600")[3].text.strip().replace('원', '').replace(',', ''))
        number_of_investing_products = soup.select("div.p-t-10.c-client-1.f-15.f-700")[0].text.strip().replace('건','')
